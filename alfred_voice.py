...
def speak(text, sync=False):
    """
    Speaks text. 
    Use sync=True for wake-word confirmation so Alfred doesn't record his own voice.
    """
    clean = re.sub(r'\[.*?\]', '', text) # Remove [CAMERA] or [CONTINUE] tags
    clean = re.sub(r'\*', '', clean).replace('"', '').replace("'", "")
    
    if sync:
        # Blocks the code until Siri finishes talking
        subprocess.run(["say", "-v", "Siri", clean])
    else:
        # Runs in background so the HUD doesn't freeze
        def run(): subprocess.run(["say", "-v", "Siri", clean])
...
def listen_ambient_loop():
    ...
    while True:
        # Only listen if Alfred isn't already busy
        if STATUS == "IDLE":
            try:
                # Capture 2.5s of audio. -q keeps terminal clean. -r 16000 ensures compatibility.
                subprocess.run(["rec", "-q", "-c", "1", ambient_file, "rate", "16k", "trim", "0", "2.5"], check=True)
                
                # Transcribe using the PRE-LOADED whisper_brain
                res = mlx_whisper.transcribe(ambient_file, path_or_hf_repo=WHISPER_MODEL)
                text = res['text'].lower()
...
def trigger_voice_interaction():
    ...
    STATUS = "LISTENING"
    speak("Yes? How can I help?", sync=True)
    
    # 2. Record the actual query (6 seconds)
    print("🎤 Recording Query...")
    query_file = "captures/query.wav"
    subprocess.run(["rec", "-q", "-c", "1", query_file, "rate", "16k", "trim", "0", "6"], check=True)
    
    # 3. Transcribe the query
    STATUS = "TRANSCRIBING"
    print("🧠 Decoding speech...")
    res = mlx_whisper.transcribe(query_file, path_or_hf_repo=WHISPER_MODEL)
